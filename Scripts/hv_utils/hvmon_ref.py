...
def printHeader():
    print(st.generate_data_row(['addr','status','Vset','V','I','T','rate UP/DN','limit V/I/T/TRIP','trigger thr','alarm', 'Vref']))
    print(st.generate_data_row(['','','[V]','[V]','[uA]','[°C]','[V/s]/[V/s]','[V]/[uA]/[°C]/[s]','[mV]','','[mV]']))
...
columns.append(Column("", width=9, data_horiz_align=HorizontalAlignment.CENTER))
    ...
    while True:
        ...
        for hv in hvList:
            try:
                ...
                print(st.generate_data_row([mon['address'], mon['status'], mon['Vset'], f'{mon["V"]:.3f}', f'{mon["I"]:.3f}', mon['T'], f'{mon["rateUP"]}/{mon["rateDN"]}', f'{mon["limitV"]}/{mon["limitI"]}/{mon["limitT"]}/{mon["limitTRIP"]}', mon['threshold'], mon['alarm'], mon['Vref']]))
