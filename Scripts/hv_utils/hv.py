...
class HighVoltageApp(cmd2.Cmd):
   ...
   rampup_parser.add_argument('value', type=functools.partial(checkRange, minVal=1, maxVal=50), help='ramp up voltage rate [V/s] (min:0 max:25)')

   rampdown_parser = rate_subparsers.add_parser('rampdown', help='rampdown rate')
   rampdown_parser.add_argument('value', type=functools.partial(checkRange, minVal=1, maxVal=50), help='ramp down voltage rate [V/s] (min:0 max:25)')
   ...
   current_parser.add_argument('value', type=functools.partial(checkRange, minVal=1, maxVal=50), help='current threshold [uA] (min:1 max:10)')

   voltage_parser = limit_subparsers.add_parser('voltage', help='voltage margin +/-')
   voltage_parser.add_argument('value', type=functools.partial(checkRange, minVal=1, maxVal=200), help='voltage margin +/- [V] (min:1 max:20)')
   ...
   def do_info(self, args: argparse.Namespace) -> None:
      ...
      self.poutput(f'{"Device ID": <25}: {info[4]}')
   ...
   #
   # calibration auto (no password)
   #
   calibration_auto_parser = argparse.ArgumentParser()

   @cmd2.with_argparser(calibration_auto_parser)
   @cmd2.with_category("High Voltage commands")
   def do_autocalibration(self, args: argparse.Namespace) -> None:
      if (self.checkConnection() is False):
         return

      self.hv.setLimitVoltage(100)
